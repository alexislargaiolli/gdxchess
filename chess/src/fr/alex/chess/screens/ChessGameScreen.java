...
public class ChessGameScreen implements Screen {
	...
	public enum GameState {
		INITIALIZATION, NEW_TURN, PIECE_SELECTION, DESTINATION_SELECTION, CHECK_MOVE, PERFORM_MOVE, WAITING_OPPONENT, PROMOTION
	}

	protected static final Vector3 cameraInitialPosition = new Vector3(30f,
			30f, 30f);
	...
	public ChessGameScreen(ChessGame game, GameInstance gameInstance) {
		...
		environment.set(new ColorAttribute(ColorAttribute.AmbientLight, 0.6f,
				0.6f, 0.6f, 1f));
		environment.add(new DirectionalLight().set(0.5f, 0.5f, 0.5f, -.9f,
				-.9f, -.9f));
		environment.add(new DirectionalLight().set(0.5f, 0.5f, 0.5f, .9f, .9f,
				.9f));
		...
		cam.position.set(chessBoard.getCenter().x, 40f,
				chessBoard.getCenter().z * .5f);
		cam.update();
		cam.lookAt(chessBoard.getCenter().x, 0f, chessBoard.getCenter().z);
		cam.near = .1f;
		cam.far = 300f;
		cam.update();
		camController.setFocus(new Vector3(chessBoard.getCenter().x, 0f,
				chessBoard.getCenter().z));
		...
		labelPlayerTurn = new Label(game.localize.getUiText("game.player.label.turn"), game.skin);
		labelOpponentTurn = new Label(game.localize.getUiText("game.opponent.label.turn"), game.skin);
		labelPlayerTurn.setPosition(w * .5f - labelPlayerTurn.getWidth() * .5f, h * .9f);
		labelPlayerTurn.setVisible(false);
		labelOpponentTurn.setPosition(w * .5f - labelOpponentTurn.getWidth()*.5f, h * .9f);
		labelOpponentTurn.setVisible(false);
		gameStage.addActor(labelPlayerTurn);
		gameStage.addActor(labelOpponentTurn);
		
		gameStage.addActor(promotionWidget);
	...
	public void update(float delta) {
		...
		case NEW_TURN:
			if (white == board.getTurn()) { // Player turn
				labelOpponentTurn.setVisible(false);
				labelPlayerTurn.setVisible(true);
				state = GameState.PIECE_SELECTION;
			} else { // Opponent turn
				labelPlayerTurn.setVisible(false);
				labelOpponentTurn.setVisible(true);
	...
	public void handleClick(float x, float y) {
		if (state == GameState.PIECE_SELECTION
				|| state == GameState.DESTINATION_SELECTION) {			
